...
{
    if (!XMLStringUtils::GetAttributeValue(pSimulationNode, "max_parallel_simulations_nb", m_iMaxNumberOfParallelSimulations))
    {
        m_iMaxNumberOfParallelSimulations = boost::thread::physical_concurrency();
    }

    BOOST_LOG_TRIVIAL(info) << "Using at most " << m_iMaxNumberOfParallelSimulations << " parallel simulations";
...
{
    XMLStringUtils::GetAttributeValue(pSimulatorsNode, "network_interfaces_file_path", m_strNetworksInterfacesFile);
...
const std::string & SimulationConfiguration::GetNetworksInterfacesFilePath() const
{
    return m_strNetworksInterfacesFile;
}
...
int SimulationConfiguration::GetMaxParallelSimulationsNb() const
{
    return m_iMaxNumberOfParallelSimulations;
}
