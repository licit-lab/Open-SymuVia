...
WalkSimulator::WalkSimulator(const SimulatorConfiguration * pConfig) :
TraficSimulatorHandler(pConfig)
{
}
...
bool WalkSimulator::Initialize(const SimulationConfiguration & config, IUserHandler * pUserHandler)
{
    m_pUserHandler = pUserHandler;
    return true;
}

bool WalkSimulator::BuildSimulationGraph(SymuCore::MultiLayersGraph *pParentGraph)
{
...
bool WalkSimulator::ActivatePPath(const PPath & currentPPath, const boost::posix_time::ptime & startTimeStepTime, const boost::posix_time::ptime & departureTime)
{
    ...
    Cost totalWalkCost;
    const Path & tripPath = currentPPath.GetTrip()->GetPath();
    ...
    for (size_t iPattern = currentPPath.GetFirstPatternIndex(); iPattern <= currentPPath.GetLastPatternIndex(); iPattern++)
    {
        ...
        totalWalkCost.plus(pPattern->getPatternCost(0, currentPPath.GetTrip()->GetSubPopulation()));
    }
